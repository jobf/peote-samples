...
// todo - fix texture coordinate scale?
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
        ...
        if(p.x < 1./3.) { // Part I
            ret = texture(uTexture1, p).xyz;
        } 
        else if(p.x < 2./3.) { // Part II
            ret = texture(uTexture1, 4.*p+vec2(0.,iTime)).xyz;
        } 
        else if(p.x < 3./3.) { // Part III
            r = r - vec2(xMax*2./3., 0.);
            float angle = iTime;
            mat2 rotMat = mat2(cos(angle), -sin(angle),
                            sin(angle),  cos(angle));
            vec2 q = rotMat*r;
            vec3 texA = texture(uTexture1, q).xyz;
            vec3 texB = texture(uTexture2, q).xyz;
...
// MOUSE INPUT
// todo - replicate vec4 of iMouse - see https://shadertoyunofficial.wordpress.com/2016/07/20/special-shadertoy-features/
//
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	float dirt = pow(texture(uTexture0, 4.0*r).x, 4.0);
